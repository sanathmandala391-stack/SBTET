      ...
      {!selected ? (
        <>
          <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:20}}>
            <input style={{padding:'8px 16px',border:'2px solid var(--border)',borderRadius:8,fontSize:14,flex:1,maxWidth:300,outline:'none'}}
              placeholder="Search name or PIN..." value={search} onChange={e => setSearch(e.target.value)} />
            <select style={{padding:'8px 16px',border:'2px solid var(--border)',borderRadius:8,fontSize:14,background:'#fff'}} value={branch} onChange={e => setBranch(e.target.value)}>
              <option value="">All Branches</option>
              {['CS','ME','CE','EE','EC','IT','CH'].map(b => <option key={b} value={b}>{b}</option>)}
            </select>
            <select style={{padding:'8px 16px',border:'2px solid var(--border)',borderRadius:8,fontSize:14,background:'#fff'}} value={semester} onChange={e => setSemester(e.target.value)}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {filtered.map(s => (
                <div key={s.id} className="card" style={{padding:'14px 20px',display:'flex',alignItems:'center',gap:14,cursor:'pointer',transition:'.2s'}}
                  ...
                  <div style={{width:40,height:40,borderRadius:'50%',background:'linear-gradient(135deg,var(--primary),var(--accent))',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,flexShrink:0}}>{s.name?.charAt(0)}</div>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600}}>{s.name}</div>
                    <div style={{fontSize:12,color:'var(--text-muted)',marginTop:2}}>{s.pinNumber} • {s.branch} {s.semester}</div>
                  </div>
        ...
        <>
          ...
          <div className="card" style={{padding:24,marginBottom:24}}>
            <h2 style={{color:'var(--primary)'}}>{selected.name}</h2>
            <p style={{color:'var(--text-muted)',marginTop:4,fontSize:14}}>{selected.pinNumber} • {selected.branch} {selected.semester}</p>
          </div>
